...
| **MIX stack particles** | **Particula-derived** multi-sphere Three.js layer on `#nx-particula-host` (MIT port from [Humprt/particula](https://github.com/Humprt/particula)); audio-reactive data is synthesized from **`NX.S`** (no extra `AudioContext` in the overlay). Vendored **Three r136** + **dat.GUI** under `vendor/particula/` — see [`docs/PARTICULA-PORT.md`](docs/PARTICULA-PORT.md). **NEXUS is not the Particula product**; keep upstream credit in distributions. |
